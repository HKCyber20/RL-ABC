# Week 01 / Day 01 作业：VLA 闭环与四类变量

[← 提交区首页](../README.md) · [本周作业](README.md) · [对应课程](../../week-01/day-01.md) · [Week 01 / Day 02 →](day-02.md)

> **提交状态：未提交。** 请直接在本文件中填写；不要修改最后的“教练验收区”，也不要自行更新 `PROGRESS.md`。

## 1. 提交信息

| 字段 | 请填写 |
|---|---|
| 实际投入时间 |  |
| 完成日期 |  |
| 执行环境 | OS、Python、PyTorch、CPU/GPU、仿真器；纯理论日写“不适用” |
| 资源等级/替代方案 | L0、L1 或 L2；若使用 mock、synthetic 或 dry-run，请明确 |
| 课程文件 | [打开当天课程](../../week-01/day-01.md) |
| 产物路径 | 代码、日志、图表或截图的相对路径 |

## 2. 今日目标与预计时间

预计 60–90 分钟。完成后能用自己的话区分 VLM 与 VLA，写出策略函数 `a_t = pi_theta(o_t, l, s_t)`，并画出包含环境反馈的闭环，而不只是一次模型前向。

### 我的目标复述

**我的回答：** observation 是外部世界观测，language 是任务指令，proprioception 是机器人状态，action 是执行器动作。

## 3. 必做作业

### 课程题目

1. 写一段不超过 150 字的 VLA 定义，并指出 VLM 与 VLA 的核心差别。
2. 为“把红色方块放进蓝色盒子”画完整闭环图。
3. 按上表设定，写出四个输入输出张量的形状和 dtype；token id 应为整数，网络输入与动作通常为浮点数。
4. 分析两种失败：视觉定位偏 2 cm；夹爪命令语义反了。说明它们如何影响下一时刻。

### 我的答案与实现

#### 第 1 题作答

**我的回答：** 通过一个策略函数，输入为环境观测，机器人状态和任务指令，输出执行器动作的模型叫做VLA

#### 第 2 题作答

**我的闭环图：**

```text
语言指令 l：“把红色方块放进蓝色盒子”（episode 内保持不变）

  (o_t：红块和蓝盒的相机图像,
   s_t：末端位姿、关节角、夹爪状态,
   l)
            |
            v
        VLA policy
            |
            v
  a_t = [Δx, Δy, Δz, Δroll, Δpitch, Δyaw, gripper]
            |
            v
  环境执行动作：靠近红块 → 抓取 → 移向蓝盒 → 松爪
            |
            v
  产生新的观测 o_{t+1} 和机器人状态 s_{t+1}
            |
            +--------反馈给 VLA，继续下一步--------+
                           |
              直到红块进入蓝盒或任务终止

  注意 a_t 应是机器人可以执行的数值动作，不能直接写成自然语言“抓起红色方块”。
```

#### 第 3 题作答

【请在这里填写变量、形状和 dtype；可以直接使用 Markdown 表格。】

#### 第 4 题作答

【请在这里分别分析“视觉定位偏 2 cm”和“夹爪命令语义反了”的因果链。】

## 4. 输入、预期输出与验证

### 课程要求

- 输入：任务描述、`B=4,H=W=224,L=16,d_s=8,d_a=7`。
- 预期输出：一张含环境反馈的图、一张完整张量表、两条因果失败链。
- 验证：闭环必须出现 `o_{t+1}` 和 `s_{t+1}`；四个 batch 维都为 4；`o_t` 通道在第二维；动作不是自然语言。

### 我的验证记录

| 检查项 | 实际结果 | 证据位置 | 是否符合 |
|---|---|---|---|
| 输入形状或数据格式 |  |  |  |
| 输出形状或数据格式 |  |  |  |
| 数值范围、dtype 或单位 |  |  |  |
| 断言、测试或评估指标 |  |  |  |
| 指标分子/分母与随机种子 |  |  |  |
| checkpoint、配置或数据哈希 |  |  |  |
| 失败分支或反例 |  |  |  |

> 未实际执行的内容必须写“预期结果”，不能写成已运行结果。

## 5. 代码、实验与运行证据

### 产物清单

| 产物 | 路径 | 用途 |
|---|---|---|
| 代码 |  |  |
| 配置 |  |  |
| 日志 |  |  |
| 图表/截图 |  |  |

### 实际运行命令

```text
# 在这里粘贴实际执行的命令；纯理论日写“不适用”
```

### 关键输出

```text
# 粘贴足以验证结果的关键输出，不要只写“运行成功”
```

退出码：

### 结果解释

- 结果说明了什么：
- 结果没有说明什么：
- 与预期不一致的地方：
- 正向测试与负向测试：
- 失败现象和定位证据：
- 可能原因及证据：

## 6. VLA 约束记录

不适用的项目写“不适用”，不要留空。

| 约束 | 本次定义 |
|---|---|
| 图像布局、颜色顺序和范围 |  |
| 文本 token 形状与 padding |  |
| 机器人状态各维含义 |  |
| 坐标系 |  |
| 长度和角度单位 |  |
| 动作空间及各维含义 |  |
| observation/action 时间对齐 |  |
| 控制频率或 action chunk |  |
| 归一化及统计量来源 |  |
| 随机种子与数据划分 |  |

## 7. 当天提交格式要求

提交 Markdown，固定包含：`一句话定义`、`闭环图`、`变量与形状表`、`两个失败链`、`仍不确定的点`、`投入分钟数`。闭环图可用 Mermaid、ASCII 或清晰手绘截图。

提交前确认本文件包含上面要求的全部内容；若课程要求与通用表格不完全一致，以课程要求为准。

## 8. 自测题

### 课程自测

1. 相机图像与末端位姿为什么不是同一种状态？
2. 哪个量的执行会改变下一帧输入？
3. 指令在 episode 中不变是否意味着模型每步都不需要它？为什么？
4. 训练集动作 MSE 很低时，仍需报告什么闭环指标？

### 我的自测答案

#### 第 1 题作答

【请在这里填写第 1 题答案】

#### 第 2 题作答

【请在这里填写第 2 题答案】

#### 第 3 题作答

【请在这里填写第 3 题答案】

#### 第 4 题作答

【请在这里填写第 4 题答案】

## 9. 常见错误、最低完成线与可选提高

### 常见错误与排查

- 把“目标位置”直接当机器人状态：先问它来自外部相机还是机器人传感器。
- 写成 `[B,H,W,C]`：PyTorch 常用 NCHW，此课按 `[B,C,H,W]`。
- 只画模型不画环境：检查动作之后是否产生下一观测。
- 认为低 loss 等于成功：检查 rollout 输入是否已偏离示范分布。

### 时间不足时的最低完成线

完成闭环图、四变量表，以及一句“为什么低 loss 不保证成功”的解释，约 30 分钟。

### 可选提高任务

给同一任务分别设计单步动作与 4 步 action chunk 的输出形状，并列出各自风险；暂不实现。

## 10. 提交前自评

- [ ] 我逐项回答了必做作业；
- [ ] 我区分了实际结果与预期结果；
- [ ] 我提供了代码、命令、日志、图表或推理过程作为证据；
- [ ] 我记录了适用的张量形状、坐标系、单位、时间对齐和动作定义；
- [ ] 我运行了当天要求的验证或明确说明未运行原因；
- [ ] 我回答了自测题；
- [ ] 我写出了至少一个仍不确定的点或失败案例。

### 仍不确定的内容或阻塞项

- 现象：
- 最小复现：
- 已尝试：
- 希望获得的帮助：

## 11. 当天评分与验收标准

总分 100，80 分通过：定义与 VLM/VLA 区分 20；闭环完整 25；四变量语义 20；形状与 dtype 20；失败因果链 15。若闭环没有反馈、混淆 `o_t/s_t` 或将动作写成文本，即使总分达到 80 也不通过。

### 我的自评分

| 项目 | 自评分 | 证据 |
|---|---:|---|
| 概念理解 |  |  |
| 实现或作业正确性 |  |  |
| 验证充分性 |  |  |
| VLA 约束记录 |  |  |
| 分析与表达 |  |  |
| 合计 | /100 |  |

> 自评分只用于检查遗漏，不能代替教练验收。

## 12. 教练验收区

> 本节由教练收到证据后填写。学习者不要预填分数、通过状态或下一课。

| 字段 | 验收结果 |
|---|---|
| 证据完整性 | 待验收 |
| Rubric 得分 | /100 |
| 门槛项 | 待验收 |
| 当天状态 | 未提交 |
| 具体缺口 |  |
| 最小补救任务 |  |
| 复验结果 |  |
| 下一课程 |  |

[← 提交区首页](../README.md) · [本周作业](README.md) · [对应课程](../../week-01/day-01.md) · [Week 01 / Day 02 →](day-02.md)
